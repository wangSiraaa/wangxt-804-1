...
import { getRoleLabel, formatTime, getStatusLabel } from "@/utils/dateUtils";
...
export default function Dashboard() {
  ...
  const { canCreateAssignment, canExport, canViewAllTechnicians } = useRoleAccess();
  ...
  const handleTestConflict = () => {
    ...
    const vh1 = vehicles[0];
    const vh2 = vehicles[1] || vehicles[0];
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">当前角色: {getRoleLabel(currentUser?.role)} | {currentUser?.name}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          ...
          <p className="text-2xl font-bold text-amber-600">
            {canViewAllTechnicians ? technicians.length : 1}
          </p>
        </div>
      </div>
                ...
                filteredAssignments.map((a) => (
                  <tr key={a.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-slate-700">
                      {getWorkstationName(a.workstationId)}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">
                      {getTechnicianName(a.technicianId)}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">
                      {getVehicleInfo(a.vehicleId)}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">
